...
void BezierCurveEvaluator::evaluateCurve(const std::vector<Point>& ptvCtrlPts, std::vector<Point>& ptvEvaluatedCurvePts, const float& fAniLength, const bool& bWrap) const
{
	...
	*/
	...
	for (i = 0; i + 3 < curve.size(); i = i + 3)
	{
		...
		//add if(adaptive) for displayBezier and the adjustable parameter (1 bell)
		...
		for (int i=0; i<SAMPLE; i++)
		{
			...
			const Vec4d T(interval* interval* interval, interval* interval, interval, 1);
			Point bezier_point(T*M*px, T*M*py);
