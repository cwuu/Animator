...
void drawTextureBox( double x, double y, double z, const char* directory)
{
	ModelerDrawState *mds = ModelerDrawState::Instance();
...
void drawTextureCylinder(double h, double r1, double r2, const char* directory)
{
	ModelerDrawState *mds = ModelerDrawState::Instance();
	int divisions;
...
void drawTriangularPolygon(double a, double b, double h, double angle)
{
	/*composition: 2 triangle surface + 3 regtangular surface 
	(each one implemented by 2 triangle surface)
	==>totally 8 triangle surface       
	*/

	drawTriangle(0, h, 0, b *cos(angle * M_PI / 180), h, b * sin(angle * M_PI / 180), a, h, 0);

	drawTriangle(0, 0, 0, a, 0, 0, b * cos(angle * M_PI / 180), 0, b * sin(angle * M_PI / 180));


	drawTriangle(0, 0, 0, b * cos(angle * M_PI / 180), 0, b * sin(angle * M_PI / 180), 0, h, 0);
	drawTriangle(b * cos(angle * M_PI / 180), 0, b * sin(angle * M_PI / 180), b * cos(angle * M_PI / 180), h, b * sin(angle * M_PI / 180), 0, h, 0);

	drawTriangle(b * cos(angle * M_PI / 180), 0, b * sin(angle * M_PI / 180), a, 0, 0, b * cos(angle * M_PI / 180), h, b * sin(angle * M_PI / 180));
	drawTriangle(a, 0, 0, a, h, 0, b * cos(angle * M_PI / 180), h, b * sin(angle * M_PI / 180));
...
void drawLSystem(int iteration) 
{
	...
	else
	{
		...
		glRotated(-100 / iteration, 0.0, 0.0, 1.0);
		glRotated(180, 0.0, 1.0, 0.0);
		drawLSystem(iteration - 1);
		glPopMatrix();
